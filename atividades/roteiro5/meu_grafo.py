...
class MeuGrafo(GrafoListaAdjacencia):
    ...
    def ha_euleriano(self):
        ...
        for v in self.N:
            if self.grau(v) % 2 != 0:
                qtd_impares += 1
                v_graus_impares.append(v)

        if self.conexo() and (qtd_impares == 0 or qtd_impares == 2):
            return [True, v_graus_impares]

        return [False, v_graus_impares]
    ...
    def caminho_euleriano(self):
        ha_caminho, graus_impares = self.ha_euleriano()

        if (not ha_caminho): return False

        grafo_copia = deepcopy(self)

        v_atual = ''
        if (len(graus_impares) == 2):
            v_atual = graus_impares[0]
        else:
            v_atual = self.N[0]
        ...
        while(not finalizada):
            ...
            arestas_incidentes = grafo_copia.arestas_sobre_vertice(v_atual)

            if (len(arestas_incidentes) == 0):
                finalizada = True
                break

            elif (len(arestas_incidentes) == 1):
                caminho.append(arestas_incidentes[0])
                v1 = grafo_copia.A[arestas_incidentes[0]].getV1()
                v2 = grafo_copia.A[arestas_incidentes[0]].getV2()

                grafo_copia.removeAresta(arestas_incidentes[0])
                
                if v1 == v_atual:
                    if grafo_copia.grau(v1) == 0:
                        grafo_copia.removeVertice(v1)
                    v_atual = v2
                    continue

                if v2 == v_atual:
                    if grafo_copia.grau(v2) == 0:
                        grafo_copia.removeVertice(v2)
                    v_atual = v1
                    continue
            
            else:
                for a in arestas_incidentes:
                    if (a in grafo_copia.A.keys() and not grafo_copia.eh_ponte(a)):
                        caminho.append(a)
